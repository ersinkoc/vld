// Regenerate REPORT.md using existing bench/result.json.
// Re-runs parity test and audit (fast), uses cached benchmark.
...
// We need to rebuild the report. The report builder is in run.mjs. Let me just import it.
// Actually, let me re-implement the report builder here to avoid loading the full run.mjs.
...
lines.push('');
lines.push('### VLD bug surfaced by parity test (1)');
lines.push('');
lines.push('| Case | Zod | VLD | Why |');
lines.push('|------|-----|-----|-----|');
lines.push('| `discriminatedUnion(...)` with `data` missing on the matched arm | reject | accept | same root cause as audit bugs #1-4: `any` fields not enforced as required |');
lines.push('');
lines.push('## 2. Deep Audit — VLD bugs surfaced');
lines.push('');
lines.push('The audit script (`audit.mjs`) runs targeted assertions on edge cases that the broader parity test does not stress. It reveals **5 cases** where VLD silently accepts inputs that Zod 4 correctly rejects. These are real VLD bugs, not design choices — required means required regardless of type.');
lines.push('');
...
lines.push('');
lines.push('In `src/validators/object.ts` (~line 590), the `passthrough` SimpleFieldMode is used for `any` / `unknown` types and writes `result[key] = fieldValue` without checking whether the field is required. Zod 4 explicitly throws `expected: nonoptional, received: undefined` for required fields.');
lines.push('');
lines.push('### Workaround until fixed in VLD');
lines.push('');
lines.push('```js');
lines.push('// Avoid relying on VLD to reject missing required any/unknown fields.');
lines.push('// Until the bug is fixed, add a refine that checks presence:');
lines.push('v.object({ data: v.any() })');
lines.push('  .refine(o => "data" in o, { message: "data is required" });');
lines.push('```');
lines.push('');
...
if (benchResult) {
  ...
  lines.push('');
  const wins = benchResult.results.filter(r => r.speedup > 1).length;
  lines.push(`VLD wins **${wins}/${benchResult.results.length}** scenarios.`);
} else {
  lines.push('Benchmark not available. Run `node examples/dropin/bench/run-bench.mjs` then re-run this report.');
}
lines.push('');
lines.push('## 4. Migration Verification');
lines.push('');
lines.push('Single source file `examples/dropin/app.mjs` was executed with both libraries:');
lines.push('');
lines.push('```bash');
...
lines.push('```');
lines.push('');
lines.push('All 17 assertions in `app.mjs` produced identical results on both libraries. (`app.mjs` is intentionally conservative — it does not exercise the audit-flagged edge cases. To reproduce the bugs in [section 2](#2-deep-audit--vld-bugs-surfaced), run `node examples/dropin/audit.mjs`.)');
lines.push('');
