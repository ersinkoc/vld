...
- Parity baseline moved to **zod 4.6.5** (`devDependency` + lockfile). The
  259-export surface, the 1400-case differential sweep, and the full test
  suite are green against it; 4.6.5 introduced no observable behavior change.
...
- **3.0.6 shipped a stale `dist/`** - the repo's `.npmrc` sets `ignore-scripts=true`,
  which also disables npm's `prepack`/`prepublishOnly` lifecycle hooks, so the
  3.0.6 tarball was packaged from a build older than the lazy-locale fix and
  `setLocaleAsync('tg')` still failed there. 3.0.7 republishes with a fresh
  build (verified: `isLocaleSupported('tg')` is true in the published
  `dist/locales/lazy`). Prefer `>= 3.0.7`; 3.0.6 is deprecated.
